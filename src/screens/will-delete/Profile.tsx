...
const Profile = (props) => {
	...
	useEffect(() => {
		...
		const userReviews: userReviewsType = [];
		db
			.collectionGroup('reviews')
			.where('user', '==', userFirestoreDocument)
			.orderBy('createdAt', 'desc')
			.get()
			.then(function (querySnapshot) {
				querySnapshot.forEach(function (doc) {
					const userReview = doc.data() as userReviewDocResponse;
					userReviews.push(userReview);
				});
				const reviewNumber: number = userReviews.length;
	...
	useEffect(() => {
		(async () => {
			const userId = props.globalState.state.uid;
			const userProfileDocument = await db
				.collection('userList')
				.doc(userId)
				.get()
				.then(function (doc) {
			...
			userProfileDocument.iconURL != 'test-url'
				? setUserIcon(userProfileDocument.iconURL)
				: setUserIcon('../../assets/okuse_yuya.jpg');
		})();
	}, []);
	// ユーザの名前を取得
	useEffect(() => {
	...
	const changeIconUrl = async (): Promise<string> => {
		const result = (await selectIconPicture()) as pickerResult;
		if (!result.cancelled) {
			let data = '';
	...
	const setIconToFirestore = (url: string): Promise<void> => {
		const userId = props.globalState.state.uid;
		return db.collection('userList').doc(userId).set(
			{
				iconURL: url,
			},
			{ merge: true },
					...
					<View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginTop: 50 }}>
						...
						<View style={{ marginLeft: 30 }}>
							<View
								...
								/>
								<ProfileNumber number={_followee} itemName="フォロー" press={toFolloweeList} />
							</View>
							<View style={{ alignItems: 'center', marginTop: 5, flexDirection: 'row' }}>
								<TouchableOpacity style={styles.editButton} onPress={tofindUser}>
									<Text style={styles.editText}>{'ユーザー検索'}</Text>
								</TouchableOpacity>
								<Icon size={20} name="cog" type="font-awesome" color="black" onPress={setting} />
							</View>
						</View>
					</View>
					<View
