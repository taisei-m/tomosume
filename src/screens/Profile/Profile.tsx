...
const Profile:React.FC<ProfileStackNavProps<'ProfileWrapper'> & ContainerProps> = (props) => {
	...
	useEffect(() => {
        ...
        const unsubscribe = db.collection('userList').doc(props.globalState.state.uid).onSnapshot(doc => {
	...
	useEffect(() => {
        ...
        const unsubscribe = db.collection('userList').doc(props.globalState.state.uid).collection('followee').onSnapshot(querySnapshot => {
            followees = []
