...
import {ContainerProps, NavLoginParamList} from 'src/types/types'
...
import { fetchShopPredictions, fetchShopDescription, registerShopInfomation, setShopInfoOnFireStore, registerReview, canPress } from './index';
...
const PostScreen:React.FC<NavigationProps & ContainerProps> = (props) => {
    ...
    const searchShop = async () => {
        if (inputedShopName == '') {
            ...
            const shopPredictions = await fetchShopPredictions(inputedShopName, _latitude, _longitude)
    ...
    const postReview = async() => {
        ...
        await registerReview(shopInfomation, props.globalState.state.uid, price, category, favoriteMenu, shopName, address)
...
export const PostScreenWrapper:React.FC<NavigationProps> = ({ navigation }) => {
	return (
			<Subscribe to={[GlobalContainer]}>
				{
					(globalState:GlobalContainer) => <PostScreen globalState={globalState} navigation = {navigation} />
				}
			</Subscribe>
	);
}
