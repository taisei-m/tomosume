...
import { userDescriptionType, UserDescriptionsType } from "src/types/types"
...
export const fetchFriendFolloweeDescriptions = async(followeeIds:string[]):Promise<UserDescriptionsType> => {
    let userDescription: userDescriptionType
    const friendFolloweeDescriptions = await Promise.all(followeeIds.map(async (docId) => {
...
export const checkFollowExchange = async(uid: string, followeeDescriptions: UserDescriptionsType):Promise<UserDescriptionsType> => {
