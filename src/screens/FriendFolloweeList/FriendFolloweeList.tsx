...
const FriendFolloweeList: React.FC<TopStackNavProps<'friendFolloweeList'> & ContainerProps> = (
	props,
) => {
	...
	useEffect(() => {
		(async () => {
			...
			const checkedFriendFolloweeDescriptions = await checkFollowExchange(
				props.globalState.state.uid,
				friendFolloweeDescriptions,
			);
...
export const FriendFolloweeListWrapper: React.FC<TopStackNavProps<'friendFolloweeList'>> = ({
	navigation,
}) => {
	return (
		<Subscribe to={[GlobalContainer]}>
			{(globalState: GlobalContainer) => (
				<FriendFolloweeList globalState={globalState} navigation={navigation} />
			)}
		</Subscribe>
	);
};
