...
export const fetchFriendFollowerDescriptions = async (
	followerIds: string[],
): Promise<UserDescriptionsType> => {
	let userDescription: userDescriptionType;
	const friendFollowerDescriptions = await Promise.all(
		followerIds.map(async (docId) => {
			await db
				.collection('userList')
				.doc(docId)
				.get()
...
export const checkFollowExchange = async (
	uid: string,
	followerDescriptions: UserDescriptionsType,
): Promise<UserDescriptionsType> => {
