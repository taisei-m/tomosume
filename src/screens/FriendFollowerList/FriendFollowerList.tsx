...
const FriendFollowerList: React.FC<TopStackNavProps<'friendFollowerList'> & ContainerProps> = (
	props,
) => {
	...
	useEffect(() => {
		(async () => {
			...
			const checkedFriendFollowerDescriptions = await checkFollowExchange(
				props.globalState.state.uid,
				friendFollowerDescriptions,
			);
...
export const FriendFollowerListWrapper: React.FC<TopStackNavProps<'friendFollowerList'>> = ({
	navigation,
}) => {
	return (
		<Subscribe to={[GlobalContainer]}>
			{(globalState: GlobalContainer) => (
				<FriendFollowerList globalState={globalState} navigation={navigation} />
			)}
		</Subscribe>
	);
};
