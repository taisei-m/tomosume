...
import { UserDescriptionsType, ProfileStackNavProps, ContainerProps } from '../../types/types';
import { styles } from './style';
import { checkFollowExchange, showFrinedCandidates } from './index';

const UserSearch: React.FC<ProfileStackNavProps<'UserSearchWrapper'> & ContainerProps> = (
	props,
) => {
	...
	const [_candidateUsersList, setCandidateUsersList] = useState<UserDescriptionsType>();

	useEffect(() => {
		(async () => {
			const candidateUserDescriptions = await showFrinedCandidates(value);
			const candidates = await checkFollowExchange(
				props.globalState.state.uid,
				candidateUserDescriptions,
			);
...
export const UserSearchWrapper: React.FC<ProfileStackNavProps<'UserSearchWrapper'>> = ({
	navigation,
}) => {
	return (
		<Subscribe to={[GlobalContainer]}>
			{(globalState: GlobalContainer) => (
				<UserSearch globalState={globalState} navigation={navigation} />
			)}
		</Subscribe>
	);
};
